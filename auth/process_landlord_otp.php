...
if (!$user || $user['otp_status'] !== 'sent') {
    header("Location: verify_account.php?error=invalid_request"); 
    exit;
}
...
if (password_verify($enteredOtp, $user['otp_hash'])) {
    ...
    $success = $db->prepare("UPDATE users SET otp_status = 'passed', otp_attempts = 0 WHERE user_id = ?");
    $success->execute([$userId]);
    
    
    header("Location: verify_account.php?success=verified");
    exit;
